...
local function SetProps(Element, Props)
	-- pcall: executor identity can be wrong after game-context switches, and
	-- assigning Instance props (Parent, etc.) throws "lacking capability Plugin".
	if typeof(Element) ~= "Instance" or type(Props) ~= "table" then
		return Element
	end
...
local function SetChildren(Element, Children)
	if typeof(Element) ~= "Instance" or type(Children) ~= "table" then
		return Element
	end
...
function OrionLib:MakeNotification(NotificationConfig)
	spawn(function()
		-- UI Instance creation needs executor identity. Trainer loops switch the
		-- calling thread to game context (2); spawn inherits that and throws
		-- "lacking capability Plugin" when parenting notification frames.
		pcall(function()
			if type(_G.F) == "table" and type(_G.F.jackEnterExecutorContext) == "function" then
				_G.F.jackEnterExecutorContext()
			elseif type(setthreadidentity) == "function" then
				setthreadidentity(8)
			elseif type(set_thread_identity) == "function" then
		...
		local okNotify, errNotify = pcall(function()
			...
			if typeof(NotificationParent) ~= "Instance" or NotificationParent.Parent == nil then
				error("notification parent failed")
			end
		...
		if not okNotify then
			warn("[Orion] MakeNotification failed:", errNotify)
		end
	end)
end
